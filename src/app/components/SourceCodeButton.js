...
export default function SourceCodeButton({ project, className = "" }) {
  ...
  const isWebflow = 
    project.id === "amply" || 
    project.tech?.some(t => t.toLowerCase().includes("webflow")) ||
    project.category?.toLowerCase().includes("webflow");
  ...
  return (
    <>
      ...
      {/* Premium Floating Portal-style Toast Notification */}
      <AnimatePresence>
        {showToast && (
          <div className="fixed top-6 right-6 z-[100000] w-full max-w-[90vw] sm:max-w-[420px] pointer-events-auto">
            <motion.div
                ...
                <div className="flex-grow space-y-1">
                  ...
                  <div className="flex items-center space-x-2.5 pt-3">
                    <Button
                      ...
                      className="bg-emerald-500 hover:bg-emerald-600 text-neutral-950 font-bold rounded-lg px-3 py-1.5 h-auto text-[10px] sm:text-xs flex items-center gap-1 hover:scale-[1.02] active:scale-[0.98] transition-transform duration-200 cursor-pointer shadow-md shadow-emerald-500/10"
                    >
                    ...
                    <button
                      onClick={() => setShowToast(false)}
                      className="text-[10px] sm:text-xs font-bold text-neutral-400 hover:text-white transition-colors cursor-pointer px-2 py-1"
                    >
                      Dismiss
                    </button>
                  </div>
